        ...
        <div className="flex items-center justify-center">
          <div
            className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-2xl border text-xs sm:text-sm font-semibold shadow-xs transition-all ${
              isPro
                ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-300 dark:border-emerald-500/30 text-emerald-900 dark:text-emerald-300 ring-4 ring-emerald-500/10'
                : 'bg-slate-100 dark:bg-[#141417] border-slate-200 dark:border-[#27272a] text-slate-700 dark:text-zinc-300'
            }`}
          >
            {isPro ? (
              <>
                <Crown size={18} className="text-emerald-600 dark:text-emerald-400 shrink-0 fill-emerald-600 dark:fill-emerald-400" />
                <span>
                  Tu plan actual: <strong className="font-bold text-emerald-950 dark:text-emerald-200">Pro</strong>
                  {company?.plan_expires_at && (
                    <span className="font-normal text-emerald-800 dark:text-emerald-400 ml-1">
                      (vence el{' '}
              ...
              <>
                ...
                <span>
                  Tu plan actual: <strong className="font-bold text-slate-900 dark:text-white">Free</strong>
                </span>
              </>
            )}
          </div>
        ...
        >
          {displayedPlans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative flex flex-col rounded-3xl bg-white dark:bg-[#141417] transition-all duration-200 h-full ${
                  isPlanAnual
                    ? 'border-2 border-blue-600 dark:border-blue-500 shadow-[0_20px_45px_-10px_rgba(27,79,216,0.25)] ring-4 ring-blue-500/10 dark:ring-blue-500/20'
                    : isButtonActive
                    ? 'border-2 border-emerald-500 dark:border-emerald-500/80 shadow-lg dark:shadow-emerald-950/20'
                    : 'border border-slate-200 dark:border-[#27272a] shadow-sm hover:shadow-md dark:hover:border-[#3f3f46]'
                }`}
              >
                {/* Top Badge */}
                {isPlanAnual ? (
                  ...
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-[#1f1f23] text-white text-[10px] font-extrabold uppercase tracking-widest px-3 py-0.5 rounded-full shadow-md flex items-center gap-1 whitespace-nowrap z-10 border border-transparent dark:border-[#27272a]">
                    <Zap size={11} className="text-amber-400 fill-amber-400" />
                    <span>Más Popular</span>
                  </div>
                ) : isPlanSemestral ? (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-indigo-900 dark:bg-[#1f1f23] text-white text-[10px] font-extrabold uppercase tracking-widest px-3 py-0.5 rounded-full shadow-md flex items-center gap-1 whitespace-nowrap z-10 border border-transparent dark:border-[#27272a]">
                    <Sparkles size={11} className="text-blue-300" />
                    <span>Recomendado</span>
                  </div>
                ) : isPlanMensual ? (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-slate-800 dark:bg-[#1f1f23] text-white text-[10px] font-extrabold uppercase tracking-widest px-3 py-0.5 rounded-full shadow-md flex items-center gap-1 whitespace-nowrap z-10 border border-transparent dark:border-[#27272a]">
                    <span>Flexible</span>
                  </div>
                ) : isButtonActive ? (
                ...
                {/* Card Header & Pricing */}
                <div className="p-5 border-b border-slate-100 dark:border-[#27272a] flex flex-col pt-6">
                  <div className="flex items-start justify-between gap-1.5 min-h-[42px]">
                    ...
                    {!isPlanFree ? (
                      <span className="p-1.5 rounded-xl bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-500/25 shrink-0">
                        <Sparkles size={14} />
                      </span>
                    ) : (
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-[#1f1f23] px-1.5 py-0.5 rounded shrink-0">
                        Básico
                      </span>
                    )}
                  </div>

                  <p className="text-[11.5px] text-slate-500 dark:text-zinc-400 mt-1 min-h-[38px] leading-relaxed">
                    {plan.tagline}
                  </p>
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100 dark:border-[#27272a]">
                    <div className="flex items-baseline gap-1">
                      <span className="font-display text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                    ...
                    <div className="mt-1 min-h-[22px] flex items-center">
                      <span className="text-[10.5px] font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-500/15 border border-transparent dark:border-blue-500/20 px-2 py-0.5 rounded-md leading-tight">
                        {plan.priceEquiv}
                      </span>
                    </div>
                    ...
                    <div className="min-h-[20px] mt-1 flex items-center">
                      {plan.highlightDiscount ? (
                        <span className="text-[9.5px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/25 px-1.5 py-0.5 rounded-md leading-tight">
                          🎁 {plan.highlightDiscount}
                        </span>
                      ) : null}
                    </div>
                ...
                <div className="p-5 bg-slate-50/40 dark:bg-transparent rounded-b-3xl flex-1 flex flex-col justify-between space-y-5">
                      ...
                      {plan.features.map((feat, idx) => (
                        <li
                          ...
                          className={`flex items-start gap-2 ${
                            !feat.included ? 'text-slate-400 dark:text-zinc-600 line-through opacity-70' : ''
                          }`}
                        >
                          {feat.included ? (
                            <div
                              className={`w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                                feat.highlight
                                  ...
                                  : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 dark:border dark:border-emerald-500/30'
                              }`}
                            >
                              <Check size={9} strokeWidth={3} />
                            </div>
                          ) : (
                            <div className="w-3.5 h-3.5 rounded-full bg-slate-200 dark:bg-[#27272a] text-slate-500 dark:text-zinc-500 flex items-center justify-center shrink-0 mt-0.5">
                              <CloseIcon size={9} strokeWidth={2.5} />
                            </div>
                          )}
                          <span
                            className={`text-[11px] leading-snug ${
                              feat.highlight ? 'font-semibold text-slate-900 dark:text-white' : ''
                            }`}
                          >
                            {feat.text}
                          </span>
                  ...
                  <div className="pt-3 border-t border-slate-200/60 dark:border-[#27272a] mt-auto">
                    {isPlanFree ? (
                      !isPro ? (
                        <button
                          type="button"
                          disabled
                          className="w-full py-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-300 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-default"
                        >
                        ...
                        <button
                          type="button"
                          disabled
                          className="w-full py-2.5 rounded-xl bg-slate-100 dark:bg-[#1f1f23] text-slate-400 dark:text-zinc-400 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-default"
                        >
                          Plan Básico
                        </button>
                      )
                      ...
                      <button
                        type="button"
                        disabled
                        className="w-full py-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-300 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-not-allowed opacity-90"
                      >
                        <CheckCircle2 size={15} />
                        Plan activo
                      </button>
                      ...
                      <button
                        ...
                        className={`w-full py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-150 flex items-center justify-center gap-1.5 cursor-pointer shadow-sm ${
                          isPlanAnual
                            ...
                            : 'bg-slate-900 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-white hover:bg-slate-800 text-white font-black'
